...
class User(AbstractUser, BaseModel):
    ...
    gender = models.CharField(
        max_length=1, choices=GENDER_CHOICES, default="O", verbose_name="性别"
    )
    birthday = models.DateField(null=True, blank=True, verbose_name="生日")
    introduction = models.TextField(
        max_length=500, null=True, blank=True, verbose_name="简介"
    )
...
class UserToken(BaseModel):
    ...
    user = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="tokens", verbose_name="用户"
    )
    ...
    device = models.CharField(
        max_length=200, null=True, blank=True, verbose_name="设备信息"
    )
    ...
    def save(self, *args, **kwargs):
        if not self.pk:  # 如果是新创建的token
            ...
            UserToken.objects.filter(
                user=self.user, token_type=self.token_type, is_active=True
